...
from garaga_zero.definitions import E12D, E6D, is_zero_E6D, one_E6D, one_E12D, bn, TRUE, FALSE

from garaga_zero.precompiled_circuits.final_exp_part_1_bn254 import (
    get_BN254_FINAL_EXP_PART_1_circuit,
)
from garaga_zero.precompiled_circuits.final_exp_part_2_bn254 import (
    get_BN254_FINAL_EXP_PART_2_circuit,
)
...
func final_exponentiation{
    ...
    let (local circuit) = get_BN254_FINAL_EXP_PART_1_circuit();
    ...
    let (_sum_is_zero) = is_zero_E6D(_sum, bn.CURVE_ID);

    if (_sum_is_zero == FALSE) {
        if (den_is_zero == FALSE) {
            let (circuit) = get_BN254_FINAL_EXP_PART_2_circuit();
